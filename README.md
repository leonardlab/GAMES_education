...
If you are looking for the original,fully functioning version of the code, please visit [here](https://github.com/leonardlab/GAMES) for the code associated with the following tutorial:
...
The synTF model is the example associated with the GAMES homework set and is based on a constitutively expressed (rather than chemically responsive) synTF and with a slightly different promoter activation mechanism compared to the example in the oroginal paper. 
...
4. Add an empty folder called "results" to to the src/games directory.
 
5. All code is executable using the command line
...
models/ includes the following file:
...
Descriptions of all settings in config.json
  ...
  - normalizationID: a string defining the identity of the noramalization strategy ('max' or 'mean' or 'scaling factor'). If 'scaling factor' is used, the scaling factor MUST be the last parameter in the parameters list.
...
### Model Class
The model file should include a class with the model name and should include the same general functions as the example (synTF). 
...
Users may need to run the following to add an optional dependency for 'fsspec'.
