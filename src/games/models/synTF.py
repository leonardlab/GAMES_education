...
class synTF:
    ...
    @staticmethod
    def gradient(
        y: np.ndarray, t: np.ndarray, parameters: List[float], inputs: List[float], mechanismID: str
    ) -> np.ndarray:
        ...
        if mechanismID == 'linear':
            [g, _] = parameters
            fractional_activation_promoter = g * y[1]
    ...
    def solve_experiment(self, x: List[float], dataID: str, parameter_labels: List[str]) -> List[float]:
    ...
    @staticmethod
    def normalize_data(solutions_raw: List[float], dataID: str, normalizationID: str, scaling_factor: float) -> List[float]:
        ...
        """
        if dataID == "synTF dose response":
            if normalizationID == 'max':
                solutions_norm = [i / max(solutions_raw) for i in solutions_raw]
            elif normalizationID == 'mean':
                solutions_norm = [i / np.mean(solutions_raw) for i in solutions_raw]
            elif normalizationID == 'scaling factor':
                solutions_norm = [i / scaling_factor for i in solutions_raw]
    ...
    def plot_training_data(
        ...
        x_label = "synTF protein (ng)"
        x_scale = "linear"
        y_label = "Reporter protein"
