...
def define_best_optimization_results(
    ...
    r_sq_opt = round(df_optimization_results["r_sq"].iloc[0], 4)
    chi_sq_opt_min = round(df_optimization_results["chi_sq"].iloc[0], 4)
...
def define_results_row(
    ...
    [parameter_labels, dataID, weight_by_error, normalizationID] = results_row_settings
    ...
    scaling_factor = model.parameters[-1]
    solutions_norm, chi_sq, r_sq = solve_single_parameter_set(
        x, exp_data, exp_error, dataID, weight_by_error, parameter_labels, normalizationID, scaling_factor
    )
...
def optimize_single_initial_guess(row: tuple) -> Tuple[List[Any], List[Any]]:
    ...
    results_row, results_row_labels = define_results_row(
        ...
        [parameter_labels, dataID, weight_by_error, normalizationID],
    )
