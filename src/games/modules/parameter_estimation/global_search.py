...
def solve_single_for_global_search(row: tuple) -> Tuple[List[float], float]:
    ...
    """
    # Unpack row
    model.parameters = list(row[1:-7])
    scaling_factor = model.parameters[-1]
    [x, exp_data, exp_error, dataID, weight_by_error, parameter_labels, normalizationID] = row[-7:]
...
def solve_global_search(
    ...
    df_parameters["normalizationID"] = [settings["normalizationID"]] * len(df_parameters.index)
