...
def add_noise(solutions_norm_raw: List[float], noise: List[float], dataID: str, normalizationID: str, scaling_factor: float) -> List[float]:
    """
    ...
    solutions_norm_noise = model.normalize_data(solutions_noise, dataID, normalizationID, scaling_factor)
...
def generate_pem_evaluation_data(
    ...
    for row in df_global_search_results_filtered.itertuples(name=None):
        ...
        solutions_norm_noise = add_noise(solutions_norm_raw, noise, settings["dataID"], settings["normalizationID"], model.parameters[-1])
    ...
    max_chi_sq, min_r_sq = define_pem_evaluation_criterion(r_sq_list, chi_sq_list)
    save_pem_evaluation_data(pem_evaluation_data_list)
    return pem_evaluation_data_list, max_chi_sq, min_r_sq


def define_pem_evaluation_criterion(r_sq_list: list, chi_sq_list: list) -> [float, float]:
    """Generates PEM evaluation data based on results of a global search
