...
def plot_pem_evaluation(
    df_list: List[pd.DataFrame], chi_sq_pem_evaluation_criterion: float, r_sq_pem_evaluation_criterion: float
) -> None:
    ...
    plt.savefig("PEM evaluation criterion all opt chi_sq.svg", dpi=600)

    # Plot PEM evaluation criterion
    plt.subplots(1, 1, figsize=(4, 3))
    ax2 = sns.boxplot(x="run", y="r_sq", data=df_all, color="white")
    ax2 = sns.swarmplot(x="run", y="r_sq", data=df_all, color="black")
    ax2.set(
        xlabel="PEM evaluation dataset",
        ylabel="r_sq, opt"
    )
    plt.savefig("PEM evaluation criterion all opt r_sq.svg", dpi=600)
